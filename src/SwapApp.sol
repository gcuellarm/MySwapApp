...
contract SwapApp is ReentrancyGuard{
    ...
    function swapTokens(uint256 amountIn_, uint256 amountOutMin_, address[] memory path_, uint256 deadline_) external nonReentrant{
    ...
    uint256 amountBMin_, uint256 deadline_) external nonReentrant returns(uint256, uint256, uint256){
        ...
        require(amountAMin_ <= amountADesired_ && amountBMin_ <= amountBDesired_, "Minimum amounts exceed desired amounts");
     ...
     * @param deadline_ The timestamp by which the transaction must be mined.
     */
    function removeLiquidityFromPool(address tokenA_, address tokenB_, uint256 liquidity_, uint256 amountAMin_, uint256 amountBMin_, uint256 deadline_) public nonReentrant{
        ...
        IV2Router02(V2Router02Address).removeLiquidity(tokenA_, tokenB_, liquidity_, amountAMin_, amountBMin_, msg.sender, deadline_);
     ...
     */
    function swapTokensWithFee(uint256 amountIn_, uint256 amountOutMin_, address[] memory path_, uint256 deadline_, uint256 feePercent_) external nonReentrant{
